...
import {
  ...
  COPY,
  type PhotoPlace,
} from "@/lib/marketing/punab-futsal-photocard-constants";
import {
  drawFutsalPhotoCard,
  loadPhotoCardPhoto,
  preloadPhotoCardAssets,
  type PhotoCardAssets,
} from "@/lib/marketing/punab-futsal-photocard-canvas";
import {
...
import styles from "./PunabFutsalPhotoCardGenerator.module.css";

let photoJobSeq = 0;

const PAN_PX_MAX = PHOTO_PLACE_LIMITS.panPxMax;

export function PunabFutsalPhotoCardGenerator() {
  const fileId = useId();
  const removeBgId = useId();
  const assetsRef = useRef<PhotoCardAssets | null>(null);
    ...
    void preloadPhotoCardAssets()
      ...
      .catch(() => {
        if (!cancelled) {
          ...
          setAssetsErr("Could not load the photocard background. Try Wi‑Fi or refresh.");
        }
      });
    return () => {
      cancelled = true;
  ...
  const download = useCallback(async () => {
    ...
    try {
      ...
      if (photoUrl) {
        photo = await loadPhotoCardPhoto(photoUrl);
      }
      const canvas = document.createElement("canvas");
      drawFutsalPhotoCard(canvas, { template: assets, photo, photoPlace });
      await exportJulyAwardCanvasPng(canvas, "punab-july-memorial-futsal-photocard.jpg");
        ...
        <div className="border-t border-[color:var(--color-border)] pt-6">
          ...
          {photoUrl ? (
            <div className="mt-4 space-y-4">
              ...
              <div>
                <label htmlFor="futsal-photo-zoom" className="ds-label">
                  Zoom ({Math.round(photoPlace.zoom * 100)}%)
                </label>
                <input
                  id="futsal-photo-zoom"
                  type="range"
              ...
              <div>
                <label htmlFor="futsal-photo-panx" className="ds-label">
                  Move sideways
                </label>
                <input
                  id="futsal-photo-panx"
                  type="range"
                  min={-PAN_PX_MAX}
                  max={PAN_PX_MAX}
                  step={2}
              ...
              <div>
                <label htmlFor="futsal-photo-pany" className="ds-label">
                  Move up / down
                </label>
                <input
                  id="futsal-photo-pany"
                  type="range"
                  min={-PAN_PX_MAX}
                  max={PAN_PX_MAX}
                  step={2}
            ...
            {assetsReady ? (
              <div className={styles.scaleShell} style={{ width: CARD.width * previewScale, height: CARD.height * previewScale }}>
                <section
                  ...
                  aria-label="PUNAB July Memorial Futsal Tournament 2026 photocard"
                >
