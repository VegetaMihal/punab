...
import type { BloodHeroAdminMatchRow } from "@/actions/bloodhero-admin-matches";
...
export function BloodHeroAdminMatchesContent({
  paths,
  matches,
  error,
}: {
  ...
  return (
    <div>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">Matches</h1>
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            Donor–request pairs where the donor accepted the notification.
          </p>
        </div>
              ...
              {matches.map((m) => (
                <tr key={m.id} className="align-top">
                  <td className="px-4 py-3">
                    <Link
                      href={paths.requestDetail(m.request_id)}
                      className="font-mono text-xs text-red-800 hover:underline dark:text-red-300"
                    >
                      {m.request_tracking_number ?? m.request_id}
                    </Link>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-zinc-700 dark:text-zinc-300">
                    {m.request_blood_group ?? "—"}
                  </td>
                  <td className="px-4 py-3 text-zinc-700 dark:text-zinc-300">{m.request_district ?? "—"}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className="rounded-full border border-zinc-300 bg-zinc-100 px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                      {m.request_status === "cancelled" ? "closed" : (m.request_status ?? "—")}
                    </span>
                  </td>
