...
export function BloodHeroRequestTimeline({
  ...
  return (
    <div className="relative">
      <div
        className="pointer-events-none absolute left-[11px] top-3 bottom-3 w-0.5 -translate-x-1/2 rounded-full bg-(--bh-panel) sm:top-4 sm:bottom-4"
        aria-hidden
      />
          ...
          <li key={item.key} className="relative pb-5 last:pb-0 sm:pb-6">
            <div className="flex gap-3 sm:gap-4">
              <div className="relative z-[1] flex shrink-0 justify-center pt-1 sm:pt-1.5" style={{ width: "1.375rem" }}>
                <span
                  className={`mt-0.5 h-3.5 w-3.5 shrink-0 rounded-full border-2 border-(--bh-panel) ${ring}`}
                  aria-hidden
                />
              </div>
              <div
                className={`min-w-0 flex-1 rounded-xl border px-3.5 py-3 sm:px-4 sm:py-3.5 ${
                  isCurrent
                    ? "border-(--bh-blood-tint) bg-(--bh-panel) "
                    : isDone
                      ? "border-(--bh-line) bg-(--bh-panel) "
                      : "border-dashed border-(--bh-line) bg-(--bh-panel) "
                }`}
              >
                <p className="text-base font-semibold leading-snug text-(--bh-ink) ">{item.title}</p>
                {item.detail ? (
                  <p className="mt-1.5 text-sm leading-relaxed text-(--bh-ink-soft) ">{item.detail}</p>
                ) : null}
                {item.at ? (
                  <p className="mt-2 text-xs font-medium tabular-nums text-(--bh-ink-soft) ">
