...
export default async function OrgMemberPortalLayout({ children }: { children: React.ReactNode }) {
  const { user, profile } = await getSessionProfile();
  if (!user) {
    redirect("/login?redirect=/portal/me");
  }
    ...
    <div className="flex min-h-screen flex-col bg-stone-50 dark:bg-stone-950">
      <header className="border-b border-stone-200 bg-white dark:border-stone-800 dark:bg-stone-900">
        <div className="mx-auto flex max-w-3xl flex-wrap items-center justify-between gap-3 px-4 py-3">
          <Link href="/portal/me" className="text-sm font-semibold text-stone-900 dark:text-stone-100">
            My Forum Activity
          </Link>
