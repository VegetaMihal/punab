  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-24 text-center">
      ...
      <p className="mt-3 text-sm text-stone-600 dark:text-stone-400">
        {hint ?? "A server error occurred while loading this page. Try again in a moment."}
      </p>
      {isDev && meta.code && (
      ...
      {isDev && (
        <p className="mt-8 text-left text-xs text-stone-500 dark:text-stone-500">
          Dev: check <code className="rounded bg-stone-100 px-1 dark:bg-stone-800">/api/debug/db-host</code> and{" "}
          <code className="rounded bg-stone-100 px-1 dark:bg-stone-800">src/lib/db/DATA_LAYER.md</code>.
        </p>
      )}
    </div>
  );
