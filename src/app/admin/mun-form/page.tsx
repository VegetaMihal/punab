...
import { listMunApplications, type MunApplicationRow } from "@/lib/mun-form-sheet";

export const metadata = { title: "IMUN 2026 applications" };
...
export default async function AdminMunFormPage() {
  await assertAdminScope("mun_form");

  let rows: MunApplicationRow[] = [];
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-stone-900 dark:text-stone-50">PUNAB IMUN 2026 Applications</h1>
          <p className="mt-1 text-sm text-muted">Review delegate applications and payment status.</p>
        </div>
        <a
          href="/api/admin/mun-form/export-xlsx"
      ...
      <div className="mt-8 space-y-3">
        ...
        {!error && sorted.length === 0 && (
          <EmptyState title="No applications" description="No delegate applications have been submitted yet." />
        )}
        {!error &&
          sorted.map((r) => (
            <Link
              key={r.referenceNumber}
              href={`/admin/mun-form/${r.referenceNumber}`}
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted">
                  {r.referenceNumber} · {r.institutionName} · {new Date(r.submittedAt).toLocaleDateString("en-GB")}
                </p>
              </div>
