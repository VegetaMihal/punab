...
import { ASSETS, CARD, EXPORT_SCALE, PHOTO_WELL, type PhotoPlace } from "@/lib/marketing/punab-futsal-photocard-constants";

export const PHOTOCARD_W = CARD.width;
export const PHOTOCARD_H = CARD.height;

export type PhotoCardDrawInput = {
...
/** CSS preview ↔ canvas export contract — mirrors the facecard generator. */
...
/** Draw the full futsal photocard onto `canvas` at export resolution — background art is a single flattened image. */
export function drawFutsalPhotoCard(canvas: HTMLCanvasElement, input: PhotoCardDrawInput): void {
  const { template, photo, photoPlace } = input;
  canvas.width = PHOTOCARD_W * EXPORT_SCALE;
  canvas.height = PHOTOCARD_H * EXPORT_SCALE;
...
export type PhotoCardAssets = {
  bg: HTMLImageElement;
};

/** Static art preloaded once on mount. */
export async function preloadPhotoCardAssets(): Promise<PhotoCardAssets> {
  const bg = await loadJulyAwardImage(ASSETS.bg);
  return { bg };
}

export async function loadPhotoCardPhoto(src: string): Promise<HTMLImageElement> {
