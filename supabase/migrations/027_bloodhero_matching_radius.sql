-- BloodHero matching: cap donor distance at 25 km from the request donation point.
-- Donors missing coordinates (either side) are still eligible via the district fallback.
-- Keep in sync with BLOODHERO_MAX_MATCH_RADIUS_KM in src/lib/bloodhero/distance.ts.
-- Otherwise identical to 025_bloodhero_matching_distance_tiebreak.sql.
-- Rollback: re-apply 025_bloodhero_matching_distance_tiebreak.sql.
...
begin
  ...
  ranked as (
    select donor_id
    from eligible
    where distance_km is null or distance_km <= 25
    order by
...
comment on function public.bloodhero_run_matching_for_request(uuid) is
  'Selects up to 15 eligible donors (active/approved, same blood group, not blocked), within 25 km, ranked by cooldown then distance to donation point (district as fallback when coords missing) then signup order; advisory-locked per request.';
